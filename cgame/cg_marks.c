...
*/
//Yorvik
const int MARK_FADE_TIME	= 1000;	//time taken to fade out
//Y

//Yorvik - changed this lots
//
//cg_addmarks is now a timer rather than a toggle - if it is less than zero, the marks never 
//fade, otherwise thay take cg_addmarks->value seconds to fade out
//
void CG_AddMarks( void ) {
	...
	int			t;
	int			fade;
	int			time = cg_addMarks->value*1000;
	...
	for ( ; mp != &cg_activeMarkPolys ; mp = next ) {
		...
		if ( cg_addMarks->value >= 0 && cg.time >= mp->time + time ) {
			CG_FreeMarkPoly( mp );
			continue;
		}

		// fade all marks out with time
		t = mp->time + time - cg.time;
