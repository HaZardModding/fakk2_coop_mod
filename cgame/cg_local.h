...
#define	MAX_MARK_POLYS		1024 //Yorvik: was 64
...
typedef struct {
	...
	//Yorvik
	int			ping;				//ping (in milliseonds
	int			health;				//how much health we got
	int			water;				//how much water we got
	int			team;				//team we are on
	int			time;				//time (in minutes) since we connected
	int			frags;				//how many frags we got
	int			caps;				//how many times we captured the flag
	int			lastkiller;			//number of last client that killed us
	int			location;			//index of our current location
	qboolean	flag;				//do we have a flag
	qboolean	intermissionReady;	//are we 'ready'
	int			techs[MAX_TECHS];	//the id nums of the techs we have
	vec3_t		teamOverlayColor;	//color based on dist from red/blue bases
	//Y
...
typedef struct {
   ...
   qhandle_t   levelExitShader;
	...
	//Yorvik
	qhandle_t	CTFFlagShaders[6];
	qhandle_t	ui_charset;
	qhandle_t	ui_numset;
	qhandle_t	ui_healthicon;
	qhandle_t	ui_watericon;
	qhandle_t	ui_watericon_tiny;
	qhandle_t	ui_healthicon_tiny;
	qhandle_t	ui_healthicon_tiny_low;
	qhandle_t	ui_healthicon_tiny_dead;
	qhandle_t	ui_gunclip;

	qhandle_t	sb_lfade;
	qhandle_t	sb_rfade;

	qhandle_t	lagometerError;
	qhandle_t	unEvenTeamWarning;
	qhandle_t	hudbar;
...
typedef struct {
   ...
   int            cinematic;
	...
	//Yorvik
	int				capturelimit;
	vec3_t			redBaseOrigin;
	vec3_t			blueBaseOrigin;
	//Y
...
extern cvar_t* cg_drawPing;
extern cvar_t* cg_drawTime;
...
//
// cg_ui.h
void CG_JoinArenaUI_f( void );
void CG_RefreshArenaUI_f( void );
void CG_HudPrint_f( void );
void CG_MessageMode_f( void );
void CG_StatusUI_f( void );
void CG_InitArenaUI_f( void );
void CG_JoinTeamUI( int team );
// Wilka
void CG_InitCTF_UI_f(void);
// W
//Yorvik
void CG_UpdateTechs_UI_f(void);
void CG_ShowHostGameMenu_UI_f(void);
void CG_JoinTeamMenu(void);
void CG_HudPrint_UI_f(void);
void CG_TeamHudPrint_UI_f(void);
void CG_DrawText_UI_f(void);
void CG_CenterPrint_UI_f(void);
void CG_ScoreboardPrint_UI_f(void);
