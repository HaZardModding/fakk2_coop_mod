...
Event EV_CTF_SetTechIconShader	("techiconshader",	EV_DEFAULT,	"s", NULL, "name of the shader used to draw the tech icon on the client hud");
Event EV_CTF_PickupIcon			("pickup_icon",		EV_DEFAULT,	"s", NULL, "set the icon for the given item");
//Y

CLASS_DECLARATION( Listener, ClientGameCommandManager, NULL )
   {
	//HZM Coop Mod Chrissstrahl - Code Compatibilty Fix - added & for compatibility
	//{&EV_CTF_SetTechIconShader,		&CTF_SetTechIconShader	}, - fixed multiple occurences

	//Yorvik
		{&EV_CTF_SetTechIconShader,		&CTF_SetTechIconShader	},
		{&EV_CTF_PickupIcon,			&CTF_SetPickupIcon		},
	//Y
...
//void(*endblockfcn)(void) { ClientGameCommandManager::EndTagSpawn };
//(*endblockfcn) void;
...
CLASS_DECLARATION( Listener, EmitterLoader, NULL )
   { 
		//HZM Coop Mod Chrissstrahl - Code Compatibilty Fix
		//{ &EV_Remove,				Listener::Remove }, - fixed multiple occurences
...
//Yorvik
void ClientGameCommandManager::CTF_SetTechIconShader(Event* ev)
{
	CTF::SetTechIconShader(ev->GetInteger(1), ev->GetString(2));
}

void ClientGameCommandManager::CTF_SetPickupIcon(Event* ev)
{
	CTF::GetPickupManager().AddItemInfo(
		ev->GetString(1),
		cgi.R_RegisterShaderNoMip(ev->GetString(2))
	);
}
//Y