...
Yorvik: modified to fit on virtual screen
==============
*/
void CG_DrawDisconnect( void ) {
	...
	h = LAGOMETER_HEIGHT;
	x = SCREEN_W-w;
	y = SCREEN_H-(h+HUD_BAR_HEIGHT);

	CTF::DrawPic(x, y, w, h, 0, 0, 1, 1, cgs.media.lagometerError);
}
...
float CG_DrawLagometer( void ) {
	...
	if ( !cg_lagometer->integer )
      	return SCREEN_H-HUD_BAR_HEIGHT;

	aw = LAGOMETER_WIDTH;
	ah = LAGOMETER_HEIGHT;
	ax = SCREEN_W-aw;
	ay = SCREEN_H-(ah+HUD_BAR_HEIGHT);

	CTF::SetColor(1,1,1,1);
	CTF::DrawPic(ax, ay, aw, ah, 0, 0, 1, 1, cgs.media.lagometerShader);
	...
	for ( a = 0 ; a < aw ; a++ ) {
		...
		if ( v > 0 ) {
			...
			CTF::DrawBox( ax + aw - a, ay + ah - v, 1, v );
		} else if ( v < 0 ) {
			if ( color != 4 ) {
			...
			CTF::DrawBox( ax + aw - a, ay + ah - range, 1, range );
		}
	}
