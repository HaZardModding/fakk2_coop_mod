...
//Yorvik - basically rewrote this entire func
void CG_Player( centity_t *cent )
{
	//calc the client num of this player
	int clientNum = cent-cg_entities;

	//dont draw icons over your own head or spectators heads - EVER
	if(clientNum == cg.snap->ps.clientNum || cgs.clientinfo[clientNum].team == TEAM_NONE)
		return;

	float offset = 112.0f;

	//team icons above teammates heads (not yours tho)
	if(cgs.clientinfo[clientNum].team == cgs.clientinfo[cg.snap->ps.clientNum].team)
	{
		CTF::HeadSprite(cent, cgs.media.teamarrow, offset);
		offset += HEAD_ICON_SIZE;
	}

	//capture
	if(cent->currentState.eFlags & EF_CTF_CAPTURE)
	{
		CTF::HeadSprite(cent, cgs.media.capture, offset);
		offset += HEAD_ICON_SIZE;
	}
	
	//defend
	if(cent->currentState.eFlags & EF_CTF_DEFEND)
	{
		CTF::HeadSprite(cent, cgs.media.defend, offset);
		offset += HEAD_ICON_SIZE;
	}

	//assist
	if(cent->currentState.eFlags & EF_CTF_ASSIST)
	{
		CTF::HeadSprite(cent, cgs.media.assist, offset);
		offset += HEAD_ICON_SIZE;
	}
