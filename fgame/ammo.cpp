...
Item *AmmoEntity::ItemPickup
   ...
   player = ( Sentient * )other;
	...
	//Yorvik - make sure we arent full first
	if(g_gametype->integer == GT_CTF)
	{
		if(Ammo* ammo = player->FindAmmoByName(item_name))
		{
			if(ammo->getAmount() >= player->MaxAmmoCount(item_name))
				return NULL;
		}
...
//Yorvik
void AmmoEntity::CTF_DisplayPickup(Entity* ent)
{
	if(!ent)
	...
	gi.SendServerCommand(ent->edict-g_entities, "item_pickup ammo %s", getName().c_str());
}