...
namespace CTF{
	qboolean Say				(gentity_t*);
	qboolean SayTeam			(gentity_t*);
	qboolean ReturnSpitModule	(gentity_t*);
	qboolean DropFlag			(gentity_t*);
	qboolean DropTech			(gentity_t*);
	qboolean HostGameMenu		(gentity_t*);
	qboolean UpdateScores		(gentity_t*);
	qboolean UpdateLocations	(gentity_t*);
	qboolean CallVote			(gentity_t*);
	qboolean Vote				(gentity_t*);
	qboolean InitClientData		(gentity_t*);
	qboolean FollowNextTeammate	(gentity_t*);
	qboolean FollowOff			(gentity_t*);
}//~namespace
//Y
...
consolecmd_t G_ConsoleCmds[] =
   ...
   //   command name		function             available in multiplayer?
	...
	//CTF
	  {"ctf_initclientdata",CTF::InitClientData,	qtrue},
	  {"ctf_spit_return",	CTF::ReturnSpitModule,	qtrue},
	  {"ctf_drop_flag",		CTF::DropFlag,			qtrue},
	  {"ctf_drop_tech",		CTF::DropTech,			qtrue},
	  {"say_team",			CTF::SayTeam,			qtrue},
	  {"say",				CTF::Say,				qtrue},
	  {"updatescores",		CTF::UpdateScores,		qtrue},
	  {"updatelocations",	CTF::UpdateLocations,	qtrue},
	  {"callvote",			CTF::CallVote,			qtrue},
	  {"vote",				CTF::Vote,				qtrue},
	  {"follow_teammate",	CTF::FollowNextTeammate,qtrue},
	  {"follow_off",		CTF::FollowOff,			qtrue},
	  {"balance_teams",		CTF::BalanceTeams,		qtrue},
	//C
      ...
      if ( ent && !G_ProcessClientCommand( ent ) )
         {
		  ...
		  //Yorvik - use the ctf say code instead
...
void G_Say
   ...
   if ( !ent->entity )
	...
	if ( team )
		{
		Com_sprintf( text, sizeof( text ), "(%s): ", CTF::GetName(ent) );
		}
	else
		{
		Com_sprintf( text, sizeof( text ), "%s: ", CTF::GetName(ent) );
   ...
   if ( dedicated->integer )
		{
      gi.SendServerCommand( NULL, "stufftext \"hudprint \"%s\"\"", text );
		}
   ...
   if ( g_gametype->integer == GT_ARENA )
   ...
   else
      {
   	for( j = 0; j < game.maxclients; j++ )
         ...
         gi.SendServerCommand( other-g_entities, "stufftext \"hudprint \"%s\"\"", text );
         }
		}
	}
