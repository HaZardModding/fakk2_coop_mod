...
static Entity *FindClosestEntityInRadius
   ...
   while( ent )
      {      
		...
		//Yorvik - special auto aim handling for players
		if(g_gametype->integer == GT_CTF && ent->isSubclassOf(Player))
		{
			valid_entity = (static_cast<Player*>(ent)->takedamage == DAMAGE_AIM);
		}
		else
		{
...
void Player::SwipeOn
   ...
   if ( weapon && ( water_power >= water_required ) )
	...
	{
		orientation_t or;
		GetTagPositionAndOrientation(gi.Tag_NumForName(edict->s.modelindex, "tag_weapon_right"), &or);
		TechDamage(true, or.origin); //just for the effect
	}
	//Y
   }
