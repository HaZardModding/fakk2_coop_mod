...
void G_MoveClientToIntermission
   ...
   {
		//Yorvik - just move all clients to intermission again...
		if(g_gametype->integer == GT_CTF)
			GetCTFManager().EnterIntermission();
		//Y
   }
...
void G_CheckExitRules
...

   {
	...
	//Yorvik - use ctf managers exit checks
	if(g_gametype->integer == GT_CTF)
	{
		GetCTFManager().CheckExitRules();
		return;
	}
	//Y

   // Check whether or not to exit the level
   ...
   if ( timelimit->integer )
      {
      if ( level.time - level.startTime >= timelimit->integer*60 )
         {
			G_ExitLevel();
			return;
         }
   ...
   try
	...
	//	   }
      ...
      G_CheckExitRules();
		...
		//Yorvik - check ctf manager stuff
		if(g_gametype->integer == GT_CTF)
			GetCTFManager().RunFrame();
		//Y
      }
   ...
   try
      {
		   ...
		   if ( game.maxclients > 1 )
			   {
				//Yorvik - print to hud
				if(g_gametype->integer == GT_CTF)
					CTF::HudPrintToAll(ent->entity, va("%s entered the game\n", CTF::GetName(ent)));
				//Y
			   }
		   // Wilka
		   // Always print this message so I can see it's running correctly :)
		   
		   //Yorvik - Message of the day
			if(strlen(sv_motd->string) > 0)
				gi.SendServerCommand(ent->entity->edict-g_entities, "motd");
		   //Y
		   }
	   ...
	   //Yorvik - Call the player's ctf client begin func
	   if(g_gametype->integer == GT_CTF && ent->entity && ent->entity->isClient())
	   {
		   if(ent->entity->isSubclassOf(Player))
			   static_cast<Player*>(ent->entity)->CTF_ClientBegin();
	   }	   
	   //Y
      }
...
void G_ClientUserinfoChanged
   ...
   try
      ...
      */
      ...
      gi.setConfigstring( CS_PLAYERS + playernum, va( "name\\%s", CTF::GetName(ent) ) );
	   ...
	   if ( firstTime && game.maxclients > 1 )
         {
		   //Yorvik - print to hud
			if(g_gametype->integer == GT_CTF)
				CTF::HudPrintToAll(ent->entity, va("%s connected\n", CTF::GetName(ent)));
		   //Y
         }
