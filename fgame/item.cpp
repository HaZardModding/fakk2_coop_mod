...
Event EV_CTF_Item_ForceAmount("force_amount",	EV_DEFAULT, "f", NULL, "assures the item starts with this amount, reguardless of what 'amount' says");
Event EV_CTF_Item_SetTeam("set_team",			EV_DEFAULT, "s", NULL, "only lets members of specified team pick up this item");
Event EV_CTF_Item_ItemStay("itemstay",			EV_DEFAULT, "i", NULL, "if set to nonzero, the item does not get removed when picked up");
//Y
...
Item * Item::ItemPickup
   ...
   qboolean add_to_inventory
	...
	{
	//Yorvik
	if(g_gametype->integer == GT_CTF && spawner)
	{
   ...
   //Yorvik - dont remove if itemstay is on
	   if(g_gametype->integer == GT_CTF && !m_itemStay)
	   {
   ...
   //display the pickup on the screen
		CTF_DisplayPickup(other);
   //Y
...
qboolean Item::Pickupable
   ...
   {
	//Yorvik - check teams match
	if(g_gametype->integer == GT_CTF && !CTF_CheckTeam(other))
		return qfalse;
	//Y
...
void Item::setAmount
	...
	amount = m_gotForcedAmount ? m_forcedAmount : startamount;
...
//Yorvik
void Item::CTF_ForceAmount(Event* ev)
{
	m_forcedAmount = ev->GetInteger(1);
	m_gotForcedAmount = true;

	setAmount(-1);
}

qboolean Item::CTF_CheckTeam(Entity* other)
{
	if(m_team != TEAM_NONE && other && other->isSubclassOf(Player))
	{
		if(static_cast<Player*>(other)->GetTeam() != m_team)
			return qfalse;
	}
...
void Item::CTF_SetTeam(Event* ev)
{
	if(!ev || ev->NumArgs() < 1)
	{
		assert(false);
	...
	const char* teamString = ev->GetString(1);
	if(strlen(teamString) <= 0)
	{
		assert(false);
		return;
	}

	char ch = toupper(teamString[0]);
	switch(ch)
	{
	case 'R':
		m_team = TEAM_RED;
	...
	case 'B':
		...
		break;
	default:
		gi.DPrintf("item::set_team: invalid team - use R or B\n");
		break;
	};
}

void Item::CTF_SetItemStay(Event* ev)
{
	bool stay = false;
	
	if(ev && ev->NumArgs() >= 1)
		stay = ev->GetInteger(1) != 0;

	m_itemStay = stay;
}

void Item::CTF_DisplayPickup(Entity* ent)
{
	if(!ent)
	...
	gi.SendServerCommand(ent->edict-g_entities, "item_pickup item %s", getName().c_str());
}
