...
//Yorvik
extern Event EV_CTF_Item_ForceAmount;
extern Event EV_CTF_Item_SetTeam;
//Y
...
class Item : public Trigger
	...
	public:
	  ...
	  //Yorvik
	protected:
		bool		m_itemStay;			//if true, this item is not removed on pickup
		bool		m_gotForcedAmount;
		int			m_forcedAmount;
		teamtype_t	m_team;

		qboolean	CTF_CheckTeam(Entity* other);
		virtual void CTF_DisplayPickup(Entity* other = NULL);

	public:
	  void CTF_ForceAmount(Event* ev);
	  void CTF_SetTeam(Event* ev);
	  void CTF_SetItemStay(Event* ev);
	  //Y
