...
//Yorvik
Event EV_CTF_Door_SetTeam("set_team", EV_DEFAULT, "s", NULL, "only players of team specified (R / B) can open this door");
//Y
...
Door::Door() : m_team(TEAM_NONE) //Yorvik
	{
	float t;
   const char *text;
...
qboolean	Door::CanBeOpenedBy
   ...
   assert( master );
	...
	{
		//Yorvik - check team
		if(g_gametype->integer == GT_CTF && m_team != TEAM_NONE && ent->isSubclassOf(Player))
		{
			if(static_cast<Player*>(ent)->GetTeam() != m_team)
				return false;
		}
		//Y
...
void Door::SetTeam(Event* ev)
{
	...
	char ch = toupper(ev->GetString(1)[0]);
	switch(ch)
	{
	case 'B':
		m_team = TEAM_BLUE;
