...
//Yorvik - CTF Events
Event EV_CTF_Player_Respawn("ctf_respawn", EV_FROM_CODE, NULL, NULL, "Respawns the player.");
Event EV_CTF_Player_DeadStopAnim("deadstopanim", EV_FROM_CODE, NULL, NULL, "stops julie from animating & moving when she dies");
//Y
...
// Wilka
Event EV_Player_ChangeTeam
	(
	"ctf_team",
	EV_CONSOLE,
	"s",
   "team",
   "Change to red or blue team"
	);
// W
...
CLASS_DECLARATION(Sentient, Player, "player")
{
	...
	{ &EV_Player_Spectator,				&Spectator },
		// Wilka
	{ &EV_Player_ChangeTeam,			&ChangeTeam },
		// W
		//Yorvik
	{ &EV_CTF_Player_Respawn,			&CTF_Respawn },
	{ &EV_Damage,						&CTF_DamageEvent },
	{ &EV_Sentient_SpawnBloodyGibs,		&CTF_SpawnBloodyGibs },
	{ &EV_Killed,						&CTF_Killed },
	{ &EV_CTF_Player_DeadStopAnim,		&CTF_DeadStopAnim },
		//Y
	{ NULL, NULL }
};
...
qboolean Player::checkattackleft
   ...
   if ( ( g_gametype->integer == GT_ARENA || g_gametype->integer == GT_CTF ) && !allow_fighting )
      {
      return false;
      }
...
Player::Player() : 
...

	{
		...
		for(int i=0; i<static_cast<int>(CTF::REWARD_MAX); ++i)
			m_rewards.push_back(std::make_pair(0, -999.9f));
		//Y
   ...
   if ( !LoadingSavegame )
      {
	   //Yorvik - do this later on; once the team is known
...
void Player::Init
	...
	//Yorvik - call CTF_Init for ctf games
	if(g_gametype->integer == GT_CTF)
	{
		CTF_Init();
		return;
...
void Player::InitModel
   ...
   int tagnum;
	...
	//Yorvik - init crosshair
	if(g_gametype->integer == GT_CTF)
	{
		if(crosshair)
		{
			crosshair->ProcessEvent(EV_Remove);
			crosshair = NULL;
		}

		crosshair = new Entity;
		crosshair->setModel((GetTeam() == TEAM_BLUE) ? "crosshair_blue.spr" : "crosshair_red.spr");
		crosshair->setSolidType( SOLID_NOT );
		crosshair->setMoveType( MOVETYPE_NONE );
		crosshair->takedamage = DAMAGE_NO;
		crosshair->hideModel();
...
void Player::ChooseSpawnPoint
   ...
   SelectSpawnPoint( origin, angles, thread, dm_team );
   ...
   if ( DM_ACTIVE )
      {
	   ...
	   //Yorvik
	   if(g_gametype->integer == GT_CTF)
		   CTF_Obituary(attacker, inflictor, meansofdeath);
	   //Y
...
void Player::ClientMove
   ...
   if ( !groundentity )
      {
		//Yorvik - speed player up using techs
		if(g_gametype->integer == GT_CTF)
			client->ps.speed = airspeed * TechAcro(false);

		//client->ps.speed = airspeed; <-- OLD LINE
		//Y
      }
   ...
   else
      {
		///Yorvik - speed player up using techs
		if(g_gametype->integer == GT_CTF)
			client->ps.speed = animspeed * TechAcro(false);

			//client->ps.speed = animspeed; <--OLD LINE
		//Y
...
void Player::ClientThink
   ...
   if ( ( deadflag == DEAD_DEAD && ( level.time > respawn_time ) ) )
      ...
      if ( new_buttons || ( DM_FLAG( DF_FORCE_RESPAWN ) ) )
         ...
         if ( g_gametype->integer == GT_ARENA )
            {
            // Do nothing in arena deathmatch
            }

		 //Yorvik
		 else if(g_gametype->integer == GT_CTF)
		 {
			 if(buttons & BUTTON_ATTACKRIGHT || buttons & BUTTON_ATTACKLEFT || buttons & BUTTON_USE)
				CTF_Respawn(NULL);
		 }
		 //Y
...
void Player::EventUseItem
   ...
   weaponhand_t hand = WEAPON_RIGHT;
	...
	//Yorvik - allow items names with spaces
	str name = ev->GetString( 1 );
	int i = 0;

	for(i=2; i<ev->NumArgs(); ++i)
		name += str(" ") + ev->GetString(i);
	//Y
   ...
   if ( item && item->isSubclassOf( InventoryItem ) )
      ...
      {
		//Yorvik - compensate for spaces; the hand specifier is always last arg
      hand = WeaponHandNameToNum( ev->GetString( ev->NumArgs() ) );
	  //Y
...
void Player::AcquireTarget
   ...
   if ( weapon && weapon->autoaim )
	  ...
	  //Yorvik - made angles for autoaim cone smaller
      if ( left_arm_target )
         {
         new_target  = FindEnemyInFOVFromTagWithOffset( 30 /*60*/, weapon->GetMaxRange(), L_ARM_NAME, offset );
         }
      else
         {
         new_target  = FindEnemyInFOVFromTagWithOffset( 20 /*40*/, weapon->GetMaxRange(), L_ARM_NAME, offset );
         }
	  //Y
...
void Player::UpdateStats
   ...
   if ( g_gametype->integer == GT_ARENA || g_gametype->integer == GT_CTF )
      {
...
void Player::Jump
   ...
   //Yorvik
	if(g_gametype->integer == GT_CTF)	
		maxheight = ev->GetFloat( 1 ) * TechAcro(true);
	else
...
void Player::JumpXY
   ...
   //Yorvik - added acro modifications
   float haste = 1;
   
   if(g_gametype->integer == GT_CTF)
	   haste = TechAcro(true);

   forwardmove = ev->GetFloat( 1 ) * haste;
   sidemove    = ev->GetFloat( 2 ) * haste;
   speed       = ev->GetFloat( 3 ) * haste;
   //Y
   ...
   time = distance / speed;
   velocity[ 2 ] = sv_gravity->integer * time * 0.5f * haste; //<-- Yorvik: jump higher with haste
   ...
   // Send a message to the client with the scoreboard information
	for( i = 0 ; i < game.maxclients; i++ )
      ...
      if ( gent->client )
         ...
         Com_sprintf( entry, sizeof( entry ), "%i %i %i", 
...
void Player::InitArena
   ...
   gi.SendServerCommand( edict-g_entities, "stufftext cg_initarena_ui" );

   // Spectate and put them in the lobby
   Spectator();
   InitArenaSpawnPoint();

   // Update their UI and status
   RefreshArenaUI();
...
void Player::Spectator
   ...
   // Wilka
   //HZM Coop Mod Chrissstrahl - Code Compatibilty Fix
   //SetWaterPower(int(0)); // If we have full water in spec mode, we leave a trail behind us
   SetWaterPower(float(0)); // If we have full water in spec mode, we leave a trail behind us
   // W
   }
...
void Player::WarpToPoint
   ...
   setOrigin( spawnpoint->origin + "0 0 1" );
   origin.copyTo( edict->s.origin2 );
