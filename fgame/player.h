...
class Player : public Sentient
	...
	// Include the ctf player stuff here, so we don't need 
	// to change the player file much
	...
	private:
      ...
      // Deathmatch
      int               num_deaths, num_kills;
      qboolean          spectator;
      teamtype_t        dm_team;
      qboolean          allow_fighting;
      DM_Arena          *current_arena;      
      DM_TeamPtr        current_team;
...
//      void              CheckFeet( void );
      ...
      inline void       SetDM_Arena( DM_Arena *a ){ current_arena = a; };
      inline DM_Arena * GetDM_Arena( void ){ return current_arena; };
      inline void       SetDM_Team( DM_Team *t ){ current_team = t; };
      inline DM_TeamPtr GetDM_Team( void ){ return current_team; };
