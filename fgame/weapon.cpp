...
Event EV_CTF_Weapon_ForceStartAmmo("force_startammo", EV_DEFAULT, "i", NULL, "forces starting ammo to this value");
Event EV_CTF_Weapon_ForceStartAmmoAlt("alternate_force_startammo", EV_DEFAULT, "i", NULL, "forces starting ammo (for alt fire) to this value");
Event EV_CTF_WeaponGrow("growWeapon", EV_FROM_CODE, "f", NULL, "");
//Y
...
CLASS_DECLARATION( Item, Weapon, NULL )
	{
		//Yorvik
		{&EV_CTF_Weapon_ForceStartAmmo,		SetForcedStartAmmo},
		{&EV_CTF_Weapon_ForceStartAmmoAlt,	SetForcedStartAmmoAlt},
		{&EV_Item_Respawn,					Respawn},
		{&EV_CTF_WeaponGrow,				Grow},
		//Y
...
Weapon::Weapon()
   ...
   special_move = qtrue;
	...
	//Yorvik
	INITIALIZE_WEAPONMODE_VAR(m_gotForcedStartAmmo, false);
	INITIALIZE_WEAPONMODE_VAR(m_forcedStartAmmo, 0);
	//Y
...
int Weapon::GetStartAmmo
   ...
   //Yorvik - added check for forced startammo
	if ( ( mode >= 0 ) && ( mode < MAX_FIREMODES ) )
	{
		if(g_gametype->integer == GT_CTF && m_gotForcedStartAmmo[mode])
		   return m_forcedStartAmmo[mode];
		else
...
void Weapon::Shoot
   ...
   GetMuzzlePosition( &pos, &forward, &right, &up );
	...
	//Yorvik - if we have the double damage tech, play a sound
		if(g_gametype->integer == GT_CTF)
		{
			Player* player = static_cast<Player*>(&*owner);

			if(player)
				player->TechDamage(true, pos);
		}
	//Y
...
qboolean Weapon::Drop
   ...
   if ( owner )
      {
      temp[ 0 ] = G_CRandom( 100 );
      temp[ 1 ] = G_CRandom( 100 );
      temp[ 2 ] = 200;
	...
	//Yorvik - put all our ammo in the weapon before we drop it
	if(g_gametype->integer == GT_CTF && owner && owner->isClient())
	{
		//make sure we dont use the forced startammo
		m_gotForcedStartAmmo[0] = false;
		m_gotForcedStartAmmo[1] = false;

		spawnflags |= DROPPED_PLAYER_ITEM;

		//put all ammo in the gun but dont give ammo twice
		//ie the rocket launcher sets both primary AND alternate ammo to the same thing
		if(strcmpi(ammo_type[FIRE_PRIMARY].c_str(), ammo_type[FIRE_ALTERNATE].c_str()))
		{
			startammo[FIRE_PRIMARY]		= AmmoAvailable(FIRE_PRIMARY);
			startammo[FIRE_ALTERNATE]	= AmmoAvailable(FIRE_ALTERNATE);
		}

		//half the ammo between prim and alt fire modes
		else
		{
			int ammo = AmmoAvailable(FIRE_PRIMARY);
			startammo[FIRE_PRIMARY]   = ammo/2;
			startammo[FIRE_ALTERNATE] = ammo - startammo[FIRE_PRIMARY];
		}
	}

	//make sure weapon is not still firing
		ForceIdle();
	//Y
   ...
   //Yorvik - dont respawn dropped weapons
	if(g_gametype->integer == GT_CTF)
		setRespawn(qfalse);
   //Y
...
//Yorvik - almost completely re-written this func to allow for startammo to be set by mapper
void Weapon::PickupWeapon(Event *ev)
{
	...
	if(!CTF_CheckTeam(other))
		return;
		
	Sentient* sen = static_cast<Sentient*>(other);

	//if this item was spawned by an itemspawner, let the spawner know it's been picked up
	if(spawner)
	{
	...
	//give WEAPON
	//
	if(!sen->HasItem(model))
	{
		//HACK HACK HACK
		//if we are picking up the dual uzi - make sure we also have normal uzi
		if(getName() == "DualUzi" && !sen->HasItem("Uzi"))
			sen->giveItem("weapon_uzi.tik");

		//give the weapon to the ent
		Item* item = sen->giveItem(model);

		if(!item)
			return;

		sen->ReceivedItem(item);
	}
	
	str realname = GetRandomAlias("snd_pickup");
	if(realname.length() > 1)
		sen->Sound(realname, CHAN_ITEM);
	else
		sen->Sound("sound/weapons/ammo/gunpickup.wav", CHAN_ITEM);
...
//Yorvik - set forced ammo for current firemode
void Weapon::SetForcedStartAmmo(Event* ev)
{
	if(!ev || ev->NumArgs() < 1)
	{
		assert(false);
	...
	m_gotForcedStartAmmo[FIRE_PRIMARY] = true;
	m_forcedStartAmmo[FIRE_PRIMARY] = ev->GetInteger(1);
}

void Weapon::SetForcedStartAmmoAlt(Event* ev)
{
	if(!ev || ev->NumArgs() < 1)
	{
		assert(false);
	...
	m_gotForcedStartAmmo[FIRE_ALTERNATE] = true;
	m_forcedStartAmmo[FIRE_ALTERNATE] = ev->GetInteger(1);
}

void Weapon::Respawn(Event* ev)
{
	// drop the weapon
		PlaceItem();
		link();
	...
	// Make sure we dont have an owner or a last owner
		owner = NULL;
		last_owner = NULL;
	...
	//make julie NOT look at this
		look_at_me = false;
		has_been_looked_at = true;

	//Start growing
		float curScale = edict->s.scale;
		setScale(0.0f);

		Event* grow = new Event(EV_CTF_WeaponGrow);
		grow->AddFloat(curScale);
		ProcessEvent(grow);
}

void Weapon::Grow(Event* ev)
{
	float maxscale = 1.0f;

	if(ev && ev->NumArgs() >= 1)
		maxscale = ev->GetFloat(1);

	edict->s.scale += 0.03f;

	if(edict->s.scale < maxscale)
	{
		Event* grow = new Event(EV_CTF_WeaponGrow);
		grow->AddFloat(maxscale);
		PostEvent(grow, FRAMETIME);
	}
}

void Weapon::CTF_DisplayPickup(Entity* ent)
{
	if(!ent)
	...
	gi.SendServerCommand(ent->edict-g_entities, "item_pickup weapon %s", getName().c_str());
}
//Y